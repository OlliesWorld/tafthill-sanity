    ...
    if (input) {
        ...
        try {
            ...
            if (isNodeError(error) && error.code === 'ENOENT') {
                printError(`Could not set blob ${chalk.yellow(key)} because the file ${chalk.underline(inputPath)} does not exist`);
                return;
            }
            if (isNodeError(error) && error.code === 'EISDIR') {
                printError(`Could not set blob ${chalk.yellow(key)} because the path ${chalk.underline(inputPath)} is a directory`);
                return;
            }
            printError(`Could not set blob ${chalk.yellow(key)} because the path ${chalk.underline(inputPath)} could not be read`);
            return;
        }
    }
    else if (!value) {
        printError(`You must provide a value as a command-line parameter (e.g. 'netlify blobs:set my-store my-key my value') or specify the path to a file from where the value should be read (e.g. 'netlify blobs:set my-store my-key --input ./my-file.txt')`);
        return;
    }
    if (force === undefined) {
